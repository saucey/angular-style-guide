...
@Component({
  selector: 'aegon-help',
  template: `
    <span #trigger class="help" (mouseover)="show($event)" (mouseout)="hide()" (click)="toggle($event)"></span>
...
export class HelpComponent {
  ...
  show(event): void {
    if (this.isTouch() && event.type === 'mouseover') {
      // Delegate to click event.
      return;
    }
    else {
  ...
  /*
   * Checks if window element has property
   * ontouchstart
   */
