  ...
  Drupal.behaviors.userDetailWidget = {
    ...
    getData: function () {
      ...
      function populateUserData(userData, isLoggedIn) {
        ...
        return {
          ...
          // this is actually not the username but the fullname, but since it has already available under this name, keep this designation.
          'userName': person._AE_SAMNAAM || "n.a.",
          ...
          // we need this later to verify if a mobile number is available.
          'userMobile': userData.MOBIEL || "",
    ...
    initialize: function (data) {
      ...
      var callback = function (domWidget) {
        ...
        // $(domWidget).appendTo(appendUserWidgetTo);
        that.widget = domWidget;
      };
    ...
    parseWidget: function (data, callback) {
      ...
      // $(template);
      $template = $(".user_detail_widget", context);
    ...
    hasBeenShown: function (value) {
      ...
      // Make sure that a false value is returned for every false-ish (undefined, "", 0) value.
      return $.cookie(hasBeenShownCookieName) || false;
    },
    ...
    getRelNumByType: function (type) {
      ...
      if (!this.shwRawData || !this.shwRawData.PARTIJ ||
        !this.shwRawData.PARTIJ._AE_PARTIJ_IDENTIFICATIE) {
        return null;
      }
    ...
    // If just the highest number is required, use .getAkos()[0]
    getAkos: function () {
