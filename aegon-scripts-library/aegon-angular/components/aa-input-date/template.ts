export const template = `
  <span class="aa-input-date">
      <input #dayEl class="aa-input-date__day" placeholder="dd" [required]="required" [ngModel]="day"
   ...
   --><input #monthEl class="aa-input-date__month" placeholder="mm" [required]="required" [ngModel]="month"
             type="number" min="0" inputmode="numeric" pattern="[0-9]*"
             (keydown)="keydown($event)" (keyup)="keyup('month', $event)"
             (focus)="monthEl.select()" (input)="keyup('month', $event)" (blur)="month = format(month, 2)"><!--
   --><input #yearEl class="aa-input-date__year" placeholder="jjjj" [required]="required" [ngModel]="year"
